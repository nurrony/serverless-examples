...
function register(eventBody) {
  return checkIfInputIsValid(eventBody) // validate input
    ...
    .then(
      user => (user ? Promise.reject(new Error('User with that email exists.')) : bcrypt.hash(eventBody.password, 8)) // hash the pass
    )
    .then(
      hash => User.create({ name: eventBody.name, email: eventBody.email, password: hash })
      // create the new user
    )
...
function comparePassword(eventPassword, userPassword, userId) {
  return bcrypt
    ...
    .then(
      passwordIsValid =>
        !passwordIsValid ? Promise.reject(new Error('The credentials do not match.')) : signToken(userId)
    );
}
